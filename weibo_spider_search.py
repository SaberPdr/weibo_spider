...
def GetSearchContent(key):
    ...
    item_box = driver.find_element_by_xpath("//a[@class='searchBtn' and @node-type='submit']")
    item_box.click()
    ...
    #需要抓取的开始和结束日期
    start_date = datetime.datetime(2016,11,4,0)
    end_date = datetime.datetime(2016,11,20,0)
    delta_date = datetime.timedelta(days=1)

    #需要抓取的微博类型:全部,热门,原创,认证用户或媒体
    all_type = '&typeall=1'
    hot_type = '&xsort=hot'
    ori_type = '&scope=ori'
    vip_type = '&vip=1'
    media_type = '&category=4'
    ...
    while end_stamp <= end_date:
        ...
        #通过构建URL实现每一天的查询,第二个写上类型决定搜索条件
        url = current_url + all_type + '&suball=1&timescope=custom:' + str(start_stamp.strftime("%Y-%m-%d-%H")) + ':' + str(end_stamp.strftime("%Y-%m-%d-%H")) + '&Refer=g'
        driver.get(url)
...
def getContent():
    ...
    for i in range(len(nodes)):
        ...
        try:
            ZAN_TEXT = nodes[i].find_element_by_xpath(".//a[@action-type='feed_list_like']").text #可为空
            if ZAN_TEXT == '':
                ZAN = 0
            else:
                ZAN = int(ZAN_TEXT)
